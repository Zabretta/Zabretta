...
export class StatsController {
  ...
  static async getUserActivityStats(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const [
        ...
        this.getUserActivityTimeline(userId)
      ]);
  ...
  private static async getUserActivityTimeline(userId: string): Promise<any[]> {
    const days = 30;
    const timeline = [];
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      const [content, ratingAdjustments] = await Promise.all([
        prisma.content.count({
          where: {
            userId,
      ...
      timeline.push({
        ...
        content,
        ratingAdjustments,
        totalActivity: content + ratingAdjustments
      });
    }
